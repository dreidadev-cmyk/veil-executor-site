...
export default async function handler(req, res) {
  ...
  const cookieState = (req.headers.cookie || '').split(';').find(c => c.trim().startsWith('veil_oauth_state='))?.split('=')[1];
  ...
  // Build session (JWT-lite with HMAC)
  const session = {
    id: user.id,
  ...
  // Clear oauth state, set session cookie
